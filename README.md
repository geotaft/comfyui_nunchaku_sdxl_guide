...
1. Download a SDXL model that has been converted to SVDQuant. Make sure to get either the INT4 or FP4 according to your GPU architecture.
     - I have one here: https://huggingface.co/geotaft/svdq-wai-illustrious-sdxl-v14
3. Start a terminal within the folder for the ComfyUI Nunchaku custom nodes (e.g. "ComfyUI/custom_nodes/ComfyUI-nunchaku").
     - For Windows 11, you can right click inside the folder and select "Open in Terminal"
4. Run this command to obtain the code that adds nodes for SDXL support: `git fetch origin pull/680/head:sdxlsupport`
...
5. Run this command to apply the changes to the current install: `git checkout sdxlsupport`
6. After launching ComfyUI, the SDXL nodes for model loading and LORA loading should be next to the other Nunchaku nodes.
